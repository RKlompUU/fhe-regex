...
fn slices_eq(server_key: &ServerKey, a: &[RadixCiphertext], b: &[RadixCiphertext]) -> RadixCiphertext {
...
fn search(server_key: &ServerKey, content: &[RadixCiphertext], pattern: &[RadixCiphertext]) -> RadixCiphertext {
    let mut res = slices_eq(server_key, &content[..pattern.len()], pattern);

    for i in 1..=(content.len()-pattern.len()) {
        println!("i: {:?}", i);
        res = server_key.unchecked_bitor(&res, &slices_eq(server_key, &content[i..i+pattern.len()], pattern));
    }
