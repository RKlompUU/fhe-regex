...
fn encrypt_str(client_key: &ClientKey, pt: &str) -> Vec<Ciphertext> {
  pt.as_bytes().iter().flat_map(|byte| (0..8).map(|n| client_key.encrypt(*byte & (1 << n) != 0))).collect()
}

fn trivial_encrypt_str(server_key: &ServerKey, pt: &str) -> Vec<Ciphertext> {
  pt.as_bytes().iter().flat_map(|byte| (0..8).map(|n| server_key.trivial_encrypt(*byte & (1 << n) != 0))).collect()
}
